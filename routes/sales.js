...
router.get('/sales', async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Server error occurred while fetching sales data.' })
  }
})
...
router.get('/download/:id', (req, res) => {
  const sale = Sales.findById(req.params.id)
    .populate({
      ...
      select: 'name phoneNumber region' // Optionally, specify which fields to include
    })
    .populate({
      path: 'outgoings.productId', // Populate the productId field within the outgoings array
      select: 'name price' // Optionally, specify which fields to include
    })
    .populate({
      path: 'who', // Populate the who field (seller/user)
      select: 'username phoneNumber' // Optionally, specify which fields to include
    })
    .exec()
  const filePath = path.join(__dirname, `Sotuv-${sale.invoiceId}.html`)
  // Write the HTML content to a file
  fs.writeFileSync(filePath, htmlCreator(sale))

  // Send the file to the user for download
  res.download(filePath, 'download.html', err => {
    if (err) {
...
router.post('/sales', async (req, res) => {
  try {
    const { customerId, products, paymentMethod, discountApplied, totalPrice } =
      req.body
    ...
    for (const item of products) {
      ...
      if (!inventoryItem || inventoryItem.totalQuantity < item.quantity) {
        return res
          .status(400)
          .send(`Omborda mahsulot yetarli emas: ${item.productId}`)
      }
...
router.delete('/sales/:id', async (req, res) => {
  try {
    ...
    res.send({
      message: "Sotuv va unga tegishli barcha ma'lumotlar o'chirildi."
    })
  } catch (err) {
    console.error(err)
    res.status(500).send('Server error')
